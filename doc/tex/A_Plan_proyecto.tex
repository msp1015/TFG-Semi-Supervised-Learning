...
Se resuelve el error del sprint anterior de la parte de gestión de usuarios. Una vez aislado el error se ve que lo que falla es un método de una librería utilizada para encriptar contraseñas, lo que da a pensar que alguno de los parámetros pasados deja de ser compatible con la versión de la biblioteca actual. Efectivamente la versión de este proyecto de la biblioteca \textit{werkzeug} no coincidía con la del trabajo base. En concreto el parámetro incompatible que se le estaba pasando a este método es la propia contraseña encriptada, la cual empezaba por \textit{sha256}. La neuva versión del método establece que para usar \textit{sha256} el texto debe empezar por \textit{pbkdf2:sha256}, por lo tanto se cambia esta opción y también el \textit{hash} definido para el administrador al inciar la aplicación.

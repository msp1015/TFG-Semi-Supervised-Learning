...
class OwnLabelEncoder:
    ...
    def transform(self, y: DataFrame):
        """
        Transforma las etiquetas (categóricas o no) a numéricas ignorando -1 en el caso
        ...
        self.le.fit([v for v in values if v not in {-1, -1.0, "-1", "-1.0"}])

        y = pd.DataFrame([self.le.transform([v])[0] if v not in {-1, -1.0, "-1", "-1.0"} else -1 for v in values],
